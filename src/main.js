...
citySelector: 'select[name="city"]', minMaxDates: getMinMaxDates (weatherProcessor.getPeriodInDays()), cities: weatherProcessor.getCities() };
const dataForm = new DataForm(params);

const temperatureList = new TemperaturesList("items-list", "city");
function getMinMaxDates(getPeriodInDays) {
    const date = new Date();
    const start_date = date.toISOString().substring(0,10);
    const day = date.getDate();
    date.setDate(day + getPeriodInDays);
